...
| `goToStep(step: OrderStep, orderData?: IOrder)` | `step` — шаг, `orderData` — опциональные данные заказа | `void` | Переходит на указанный шаг оформления заказа (внутренний метод) |
...
2. **Factory (Фабрика)** — используется для централизованного создания объектов:
   - `CardFactory` — создание карточек товаров разных типов
   - `SuccessMessageFactory` — создание сообщений об успешном заказе
   - `OrderStep1FormFactory` — создание форм шага 1 заказа
   - `OrderStep2FormFactory` — создание форм шага 2 заказа
3. **Observer (Наблюдатель)** / **Pub/Sub** — `EventBus` (EventEmitter) для взаимодействия компонентов через события
4. **Template Method (Шаблонный метод)** — базовые классы `View`, `ViewModal` и `Form` с абстрактными методами (`render()`, `validate()`, `handleSubmit()`)
5. **Dependency Injection (Внедрение зависимостей)** — зависимости передаются через конструкторы, что обеспечивает слабую связанность и тестируемость
