...
|-------|-----------|------------|----------|
| `init()` | — | `void` | Инициализирует презентер (подписка на события, настройка коллбеков) |
| `openModal()` | — | `void` | Открывает модальное окно корзины и обновляет его данными |
...
**Поток данных:**
1. При загрузке корзины (`CART_LOADED`) → получает данные из модели → обновляет представление
2. При добавлении товара (`CART_PRODUCT_ADD`) → добавляет товар в модель → обновляет представление
3. При удалении товара (через коллбек карточки) → удаляет товар из модели → обновляет представление → эмитирует `CART_PRODUCT_DELETE`
4. При оформлении заказа → очищает корзину → эмитирует `CART_COMPLETE` с данными корзины
